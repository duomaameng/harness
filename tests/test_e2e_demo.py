...
def test_mockllm_demo_context_guardrail_feedback_repair_and_memory(tmp_path: Path):
    ...
    assert any(
        ...
        for item in result.storage.list_context_items(repo_path=str(demo_repo.resolve()))
    )
    assert any(
    ...
    assert any(not item["passed"] for item in feedback)
    assert any(item["passed"] for item in feedback)
